...
# Characters
player1 = Person('Shadow:    ', 500, 100, 80, 75, 35, [fire, ice, lightning, heal], player_items, 'Shadow')
player2 = Person('Wednesday: ', 750, 180, 50, 75, 35, [fire, ice, lightning, heal], player_items, 'Mr. Wednesday')
player3 = Person('Wisakedjak:', 600, 140, 50, 85, 35, [fire, ice, lightning, heal], player_items, 'Whiskey Jack')

players = [player1, player2, player3]

enemy = Person('Mr. World: ', 3500, 100, 40, 55, 55, [], [], 'Mr. World')
...
while is_fighting:
    ...
    for player in players:
        ...
        choice = int(input(player.display_name + ' choose an action: ')) - 1

        if choice is 0:
            is_crit = random.randrange(0, 10)
            if is_crit is 9:
                player_damage = player.generate_damage() * 3
                print(bcolors.YELLOW + '\nCritical hit!' + bcolors.ENDC, 'You strike the enemy for', str(player_damage),
                      'damage!')
    ...
    print(bcolors.RED + '\n' + enemy.display_name, 'strikes back for', str(enemy_damage), 'damage!' + bcolors.ENDC)

    if player.get_hp() is 0:
        print(bcolors.RED + '\nYou dead!', enemy.display_name, 'was victorious.' + bcolors.ENDC)
        is_fighting = False
    elif enemy.get_hp() is 0:
        print(bcolors.GREEN + '\nYou know he dead! You killed', enemy.display_name)
        is_fighting = False